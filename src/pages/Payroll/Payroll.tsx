...
export default function PayrollList() {
  ...
  { id: 30, name: "Mya Hla", payrollDate: "2025-09-30", status: "Complete", totalHours: 176.0, leaveHours: 9.0, grossPay: 400000, netPay: 380000 },
    ...
    const confirmDelete = () => {
        ...
        setData(prevData => prevData.filter(item => item.id !== payrollToDelete))
                ...
                <TableBody>
                    {currentData.map((user, index) => (
                        ...
                        >
                            ...
                            <Edit onClick={(event) => handleEdit(event, user.id)}
                                    className="text-primary-500 cursor-pointer"/>
                            <Trash2 className="text-error-400 hover:text-destructive cursor-pointer" onClick={(e) => handleDelete(e, user.id)}/>    
                            </TableCell>
                        </TableRow>
                    ))}
                </TableBody>
