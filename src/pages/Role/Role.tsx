...
const Role: React.FC = () => {
  ...
  const handelDeleteRole = () => {
    if (roleToDelete) {
      console.log(`Deleting role: ${roleToDelete.name} (ID: ${roleToDelete.no})`);
    }
    handelCloseModal();
  };
          ...
          <table className="w-full justify-between">
            ...
            <tbody>
              {rolesData.map((role: RoleType) => (
                <tr key={role.no} className="border-b odd:bg-accent even:bg-white hover:bg-accent transition-colors">
